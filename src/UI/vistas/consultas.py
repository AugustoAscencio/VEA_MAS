...
class ConsultasVer:
    ...
    def _appointment(self) -> ft.Container:
        chip = ft.Container(ft.Text("En progreso", size=10, color=Tokens.MUTED), bgcolor=ft.Colors.BLUE_GREY_50, padding=Rellenar .hv(10, 4), border_radius=12)
        row1 = ft.Row([ft.Column([ft.Text("Dra. María López", weight=ft.FontWeight.W_600, size=14), ft.Text("Cardiología", size=11, color=Tokens.MUTED)], spacing=4), chip], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
        row2 = ft.Row([ft.Icon(ft.Icons.SCHEDULE, size=16, color=Tokens.MUTED), ft.Text("Inicio 14:30 · 30 min", size=11, color=Tokens.MUTED)], spacing=8)
        btn = ft.FilledButton("Unirse a la consulta")
        return ft.Container(ft.Column([row1, row2, btn], spacing=12), bgcolor=Tokens.SURFACE, border_radius=Tokens.RADIUS, border=ft.border.all(0.5, Tokens.BORDER), padding=Rellenar .all(16))

    def build(self) -> ft.Control:
        header = ft.Row([ft.Text("Consultas", **Tokens.H2), ft.FilledButton(ft.Row([ft.Icon(ft.Icons.ADD, size=16), ft.Text("Nueva", size=12)], spacing=8))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
