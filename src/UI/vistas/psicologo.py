...
class VeaAllInOneMiskito:
    ...
    # ----------------------
    SYMPTOMS = [
        ("Wan nani painra o plaisir", "anhedonia"),
        ("Bai nani painra hela hari nain", "depresivo"),
        ("Laha painka o perdi energia", "fatiga"),
        ("No sin un sueño o mucho sueño", "sueño"),
        ("Dificultad pa concentrarse", "concentracion"),
        ("Sentimientos de inutilidad o culpa", "culpa"),
        ("Pensamientos de muerte o suicidio", "suicida"),
        ("Ansiedad mucha o preocupaciones", "ansiedad"),
        ("Agitación o irritabilidad", "irritabilidad"),
    ]
    ...
    def __init__(self, page: ft.Page):
        ...
        self.templates = {
            "greet": [
                "Ai— VEA+ yamni. Yang ai wata nani bakanka. Naha yamna nani?",
                "Yang VEA+ baka. Witin ba ai sin krah aisa bara, wina ai nani tawan yamni."
            ],
            "validate": [
                "Siksa yamni— ai nani painra takisa.",
                "Ai kawara. Nanga ai mani baka painra nani o ba sin krah?"
            ],
            "breathe": [
                "Ba sa ai sin krah: wan painra 'Start' nani."
            ],
            "positive": [
                "Yang muihkan taka yamni 😊 Ai nani baka wina painra nani?"
            ],
            "default": [
                "Siksa yamni. Ai nani baka wina painra nani o ba sin krah aisa bara?"
            ],
        }
        self.rules = {
            "triste": "Yang ai nani triste takisa. Wina ba sin krah 4-2-6 yamni.",
            "llor": "Yang ai nani prauka. Wina ai nani painra nani o muihkan sa aiska?",
            "ansio": "Yang ai nani ansiosa. Yang ba sin krah 4-2-6 yamni.",
            "angry": "Angria muihkan painra. Ba sin krah yamni."
        }
        # Palabras de riesgo (si quieres mantener la detección, se dejan en español además de miskito)
        self.risk_words = ["suicid", "no quiero vivir", "quiero morir", "lastim"]
            ...
            try:
                if left_ctrl and right_ctrl and center_ctrl:
                    ...
                    try:
                        if self.status_text:
                            if phase_name == "inhale":
                                self.status_text.value = f"Sin krah... {int(progress*100)}%"
                            elif phase_name == "hold":
                                self.status_text.value = "Sostén..."
                            else:
                                self.status_text.value = f"Exhala... {int(progress*100)}%"
    ...
    def build(self) -> ft.Control:
        ...
        title_col = ft.Column([
            ft.Text("VEA+", size=font_title, weight=ft.FontWeight.W_700, color=TEXT_LIGHT if self._dark else TEXT_DARK),
            ft.Text("Mairin Saura — ai nani painra", size=12, color=ft.Colors.with_opacity(0.8, MUTED_DARK))
        ], spacing=6)
        #close_btn = ft.ElevatedButton("Cerrar", on_click=lambda e: self.page.window_close(), bgcolor=PRIMARY)
        theme_toggle = ft.IconButton(ft.Icons.DARK_MODE, on_click=self._toggle_theme, tooltip="Alkisa tema")
        header = ft.Row([title_col, ft.Row([theme_toggle,])], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER)
        header = ft.Row([title_col,], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER)
        # Breath card
        breath_title = ft.Text("Sin krah yamni", size=18, weight=ft.FontWeight.W_600, color=TEXT_LIGHT if self._dark else TEXT_DARK)
        breath_instr = ft.Text("Painra 'Start' nani, sin krah aisa bara.", size=12, color=ft.Colors.with_opacity(0.8, MUTED_DARK))
        # widget dinámico (guardamos referencia del contenedor)
        self._breath_widget_container = self._build_breath_widget()
        self.breath_btn = ft.ElevatedButton("Start sin krah", on_click=lambda e: self._on_breath_btn(e), bgcolor=PRIMARY, width=140)
        stop_btn = ft.ElevatedButton("Stop", on_click=lambda e: self._on_stop_breath(e), width=100)
        breath_controls = ft.Row([self.breath_btn, stop_btn], spacing=12, alignment=ft.MainAxisAlignment.CENTER)
        self.status_text = ft.Text(
            "",
            size=font_medium,
        ...
        greet = self.bot_respond("")["text"]
        self.chat_list.controls.insert(0, self._make_bubble(greet, sender="bot", size=font_medium))
        self.input_field = ft.TextField(hint_text="Aiska nani… (Enter painra send)", expand=True, on_submit=lambda e: self._user_send(e))
        send_btn = ft.IconButton(ft.Icons.SEND, on_click=lambda e: self._user_send(e))
        input_row = ft.Row([self.input_field, send_btn], spacing=8)
        chat_card = ft.Container(content=ft.Column([ft.Text("Aiska", size=16, weight=ft.FontWeight.W_600, color=TEXT_DARK if self._dark else TEXT_DARK), ft.Text("Aiska bahni — yang ai baka.", size=12, color=ft.Colors.with_opacity(0.8, MUTED_DARK)), self.chat_list, input_row], spacing=10), padding=ft.padding.all(12), border_radius=14, bgcolor=ft.Colors.with_opacity(0.01, PRIMARY), margin=ft.margin.only(bottom=6))

        # Right: mood & checklist
        mood_opts = ["Sika", "Triste", "Ansiosa", "Angria", "Painra", "Neutral", "Alivia", "Esperansa"]
        self.mood_dropdown = ft.Dropdown(options=[ft.dropdown.Option(m) for m in mood_opts], hint_text="Select ai nani", expand=True)
        self.intensity_slider = ft.Slider(min=1, max=10, divisions=9, label="Intensidad", value=5, expand=True)
        self.notes_field = ft.TextField(label="Notas / ba yamni?", multiline=True, min_lines=3, max_lines=6, expand=True)
        self.tags_field = ft.TextField(label="Tags (comma painra)", expand=True)
        register_row = ft.Row([ft.ElevatedButton("Painra", on_click=lambda e: self._register_mood(e), bgcolor=ACCENT, width=140), ft.ElevatedButton("Export", on_click=lambda e: self._export_local(e), width=120)], spacing=12)
        ...
        quick_info = ft.Container(
            ft.Row(
                [
                    ...
                    ft.Column(
                        [
                            ft.Text("Evaluación wal", weight=ft.FontWeight.W_700, size=font_small),
                            ft.Text(
                                "Wal painra signal nani. Dakisa profesional sa ra baka.",
                                size=max(10, font_small-1),
                                color=ft.Colors.with_opacity(0.9, MUTED),
                            ),
                        ],
        ...
        # --- Checklist (ahora con el aviso visible encima) ---
        checks_col = ft.Column([quick_info, ft.Text("Checklist wal", weight=ft.FontWeight.W_600, size=font_medium)])
        self.symptom_checks = {}
        cb_font = self.adaptive_text_size(width, small=12, medium=13, large=14)
        for label, key in self.SYMPTOMS:
            cb = ft.Checkbox(label, value=False, expand=True, label_style=ft.TextStyle(size=cb_font))
        ...
        right_card = ft.Container(content=ft.Column([ft.Text("Ai nani painra", size=16, weight=ft.FontWeight.W_600, color=TEXT_LIGHT if self._dark else TEXT_DARK), ft.Text("Painra ba signal nani bakanka.", size=12, color=ft.Colors.with_opacity(0.8, MUTED_DARK)), self.mood_dropdown, self.intensity_slider, self.notes_field, self.tags_field, register_row], spacing=10), padding=ft.padding.all(12), border_radius=14, bgcolor=ft.Colors.with_opacity(0.01, PRIMARY), margin=ft.margin.only(bottom=6))
    ...
    def _on_breath_btn(self, e=None):
        ...
        if not self._breathing:
            ...
            if self.breath_btn:
                self.breath_btn.text = "Pausar Respiración"
                self.breath_btn.bgcolor = "#FF6F61"
            self._append_chat("Yang ai sin krah. Wina ba painra takisa.", sender="bot")
        else:
            self._stop_breath()
            if self.breath_btn:
                self.breath_btn.text = "Start sin krah"
                self.breath_btn.bgcolor = PRIMARY
            self._append_chat("Sin krah pausado. Painra aira.", sender="bot")
    ...
    def _on_stop_breath(self, e=None):
        ...
        if self.breath_btn:
            self.breath_btn.text = "Start sin krah"
            self.breath_btn.bgcolor = PRIMARY
        self._append_chat("Sin krah detenida.", sender="bot")
    ...
    def _show_typing(self):
        if not self.chat_list or self.typing_indicator:
            return
        c = ft.Container(ft.Row([ft.Text("VEA+ maya wina...", size=12, color=ft.Colors.with_opacity(0.7, MUTED_DARK)), ft.ProgressRing()]), padding=6, bgcolor=ft.Colors.with_opacity(0.02, PRIMARY), border_radius=8, margin=ft.margin.only(bottom=8))
        self.typing_indicator = c
        self.chat_list.controls.append(c)
        try:
            self.page.update()
    ...
    async def _handle_user_message(self, text: str) -> None:
        ...
        try:
            ...
            # fallback a bot local si quieres — aquí mostramos el error y luego un backup
            reply = f"Error konexión: {ex}\n\nUza respuesta lokal."
            try:
                local = self.bot_respond(text)
                reply += f"\n\n{local.get('text','Sori, ai nani error.') }"
            except Exception:
                pass
        ...
        try:
            ...
            def worker():
                ...
                try:
                    ...
                    self._append_chat("Sori, ai nani error procesando request.", sender="bot")
            threading.Thread(target=worker, daemon=True).start()
    ...
    def _register_mood(self, e=None):
        ...
        tags = [t.strip() for t in tags_raw.split(",") if t.strip()]
        entry = self.mood_add(mood, intensity, notes, tags)
        self._append_chat(f"Ai painra guardao: {mood} (Int: {intensity}). Gracias ba yula.", sender="bot")
        self.notes_field.value = ""
        self.tags_field.value = ""
        self.intensity_slider.value = 5
        self.mood_dropdown.value = None
    ...
    def _assess_symptoms(self, e=None):
        selected = [k for k, cb in self.symptom_checks.items() if cb.value]
        res = self.quick_assess(selected)
        self._append_chat(f"Evaluación wal: nivel {res['level'].upper()} (síntomas: {res['score']}). {res['advice']}", sender="bot")

    def _export_local(self, e=None):
        path = self.save_json(f"mood_export_{int(time.time())}.json", self.mood_entries)
        self._append_chat(f"Exporta lokal a: {path}", sender="bot")
    ...
    def _confirm_assessment(self, e=None):
        ...
        dialog = ft.AlertDialog(
            modal=True,
            title=ft.Text("Confirmar evaluación wal"),
            content=ft.Column(
                [
                    ft.Text("Wai va a hacer una evaluación wal. Esta valoración es orientativa y no sustituye la atención profesional."),
                    ft.Text("Wina ai nani continua?", weight=ft.FontWeight.W_700),
                ],
                spacing=8,
            ),
            actions=[
                ft.TextButton("Anular", on_click=lambda ev: self._close_dialog(ev, dialog)),
                ft.ElevatedButton("Continuar", on_click=lambda ev: self._do_assessment(ev, dialog)),
            ],
        )
        self.page.dialog = dialog
    ...
    def _do_assessment(self, ev, dialog):
        ...
        # mostrar resultado en un diálogo resumido
        result_dialog = ft.AlertDialog(
            title=ft.Text("Resultado - Evaluación wal"),
            content=ft.Column(
                [
                    ft.Text(f"Nivel: {res['level'].upper()}  (síntomas: {res['score']})"),
                    ft.Text(res['advice']),
